...
const insertPrimaryHeader = () => {
  document.getElementById('site-header').innerHTML = primaryHeaderHTML;
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  userIcon.addEventListener('click', async () => {
    ...
    // const { toggleModal } = initAuth();
    // toggleModal();
    // console.log('auth module loaded and modal toggled');

    // const authModule = await import('./auth.js');
    // const auth = authModule.initAuth(); // initAuth 실행 결과를 변수에 저장
    // auth.toggleModal(); // 반환된 객체에서 toggleModal 함수 호출

    const { initSignInForm } = await import('./auth/login.js');
    const { toggleModal } = initSignInForm();
    toggleModal();
    console.log('toggle complete');
  });

  // 장바구니 아이콘에 마우스 호버 시 미리 로드
  bagIcon.addEventListener('mouseenter', () => {
    import('./bag-claude.js'); // 프리로드'
    console.log('import complete!');
  });

  // 실제 클릭 시에는 이미 로드된 모듈 사용
  bagIcon.addEventListener('click', async () => {
    const { initBag } = await import('./bag-claude.js');
    // initBag();
    const { toggleBag } = initBag();
    toggleBag();
    console.log('using loaded module');
  });
});
